...
UEFI ファームウェアは、さまざまなハードウェアプラットフォーム上で動作する必要があります。Intel、AMD、ARM など異なる CPU アーキテクチャ、異なるチップセット、異なる周辺デバイス構成に対応するためには、**ハードウェアの詳細をソフトウェアから隠蔽する抽象化レイヤ**が不可欠です。
